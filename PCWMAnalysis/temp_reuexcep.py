...
def reuexcep(p_itimeadj, p_excep_csv, p_timesep):
    ...
    SiteID = SiteID.replace('temp_TiAd_', '')
    ...
    for excep_row in range(len_excep):
        ...
        # print(excep_row)
    if not os.path.exists('.\\temp_excep'):
       os.makedirs('.\\temp_excep')
    out_filename = 'temp_Ex_' + SiteID + '.csv'
    dataf_csv.to_csv('.\\temp_excep\\' + out_filename, index = False)
    print("temp_reuexcep: " + SiteID + "まで終了")
