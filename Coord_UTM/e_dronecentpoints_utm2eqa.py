...
data_csv = pd.read_csv(dataFolder + "/cellCent_coordUTM.csv")

# make blank data frame for the result
cols = ['id','lon', 'lat','name']
...
with tqdm(total=len(data_csv)) as pbar:
    for row in tqdm(data_csv.itertuples()):
        ...
        utm_record = pd.DataFrame({'id': row.id,'lon': utm_result[0], 'lat': utm_result[1], 'name': row.name},  index = ['1'])
        # print(utm_record)
...
# print(result_df)
result_df.to_csv(dataFolder + '/cellCent_coord.csv',index=False)